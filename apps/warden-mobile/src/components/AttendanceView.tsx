...
export default function AttendanceView({ students, onUpdateAttendance, onSubmitAllAttendance }) {
  const [activeTab, setActiveTab] = useState('mark');
  const [searchQuery, setSearchQuery] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitSuccess, setSubmitSuccess] = useState(false);
  const [history, setHistory] = useState(INITIAL_ATTENDANCE_HISTORY);
  ...
  const handleAttendanceSubmit = () => {
    ...
    if (unassignedCount > 0) {
      Alert.alert('Unmarked Students', `There are ${unassignedCount} students without attendance. Submit anyway?`, [
  ...
  const executeSubmit = () => {
    ...
    students.forEach((s) => {
      ...
      else if (s.attendanceStatus === 'leave') leaveCount++;
      else present++;
    });
    ...
    setTimeout(() => {
      ...
      const newRecord = {
        date: formattedDate,
        present: 1248 - absent - leaveCount,
        absent,
        leave: leaveCount,
      };
      const updatedHistory = [newRecord, ...history];
      setHistory(updatedHistory);
      AsyncStorage.setItem('hf_attendance_history', JSON.stringify(updatedHistory)).catch(() => {});
      onSubmitAllAttendance({ present, absent, leave: leaveCount });
    ...
    <View style={styles.screen}>
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => Alert.alert('Status', 'Attendance Service: ONLINE')}
        >
          ...
          {activeTab === 'mark' ? (
            <View>
              <View style={styles.listHeader}>
                ...
                <Text style={styles.listHeaderText}>Mark</Text>
              </View>
                  ...
                  return (
                    <View key={student.id} style={styles.studentCard}>
                      ...
                      <View style={styles.statusRow}>
                        <TouchableOpacity
                          onPress={() => onUpdateAttendance(student.id, 'present')}
                          style={[styles.statusBtn, isPresent && styles.statusPresentActive]}
                        >
                          <Icon name="check-circle" size={16} color={isPresent ? '#15803d' : colors.outline} />
                          <Text style={[styles.statusBtnText, isPresent && { color: '#166534' }]}>Present</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() => onUpdateAttendance(student.id, 'absent')}
                          style={[styles.statusBtn, isAbsent && styles.statusAbsentActive]}
                        >
                          <Icon name="close-circle" size={16} color={isAbsent ? '#b91c1c' : colors.outline} />
                          <Text style={[styles.statusBtnText, isAbsent && { color: '#991b1b' }]}>Absent</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() => onUpdateAttendance(student.id, 'leave')}
                          style={[styles.statusBtn, isLeave && styles.statusLeaveActive]}
                        >
                          <Icon name="calendar" size={16} color={isLeave ? '#a16207' : colors.outline} />
                          <Text style={[styles.statusBtnText, isLeave && { color: '#854d0e' }]}>Leave</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
                  );
                  ...
                  <View key={index} style={styles.historyCard}>
                    <View style={styles.historyCardHeader}>
                      <View style={styles.historyDate}>
                        <Icon name="format-list-checks" size={14} color={colors.primary} />
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              submitSuccess
                ...
                : isSubmitting
                  ? { backgroundColor: 'rgba(0,74,198,0.8)' }
